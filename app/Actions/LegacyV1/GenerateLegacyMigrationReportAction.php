...
class GenerateLegacyMigrationReportAction
{
    ...
    public function execute(LegacyV1ImportBatch $batch, bool $persist = false): array
    {
        ...
        $summary = [
            ...
            'total_user_mappings' => $batch->userMappings->count(),
            'resolved_user_mappings' => $batch->userMappings->where('status', 'resolved')->count(),
            'conflict_user_mappings' => $batch->userMappings->where('status', 'conflict')->count(),
            ...
            'total_legacy_orders' => $batch->orders->count(),
            'legacy_orders_paid' => $batch->orders->where('normalized_status', 'paid')->count(),
            'total_legacy_payments' => $batch->payments->count(),
            'legacy_payments_paid' => $batch->payments->where('normalized_status', 'paid')->count(),
            'total_legacy_payouts' => $batch->payouts->count(),
            'legacy_payouts_paid' => $batch->payouts->where('normalized_status', 'paid')->count(),
